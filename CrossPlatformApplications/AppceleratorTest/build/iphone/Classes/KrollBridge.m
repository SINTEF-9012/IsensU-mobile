...
NSString * AppceleratorTest1Test1$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation AppceleratorTest1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_appceleratortest1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create AppceleratorTest1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "AppceleratorTest1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_appceleratortest1 = [[AppceleratorTest1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_appceleratortest1];
	
	NSString *_appceleratortest1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _appceleratortest1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_appceleratortest1 valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:AppceleratorTest1Test1$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.appceleratortest1.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.appceleratortest1.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
