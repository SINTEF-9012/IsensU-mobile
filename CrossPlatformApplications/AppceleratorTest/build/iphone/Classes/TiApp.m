...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAppceleratorTest1UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAppceleratorTest1UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
